...
Studies of animal abundance and distribution are often conducted 
  independently of research on movement, despite the
  important links between processes. Movement can cause rapid
  changes in spatial variation in density, and 
  movement influences detection probability 
  and hence estimates of abundance from inferential methods such
  as spatial capture-recapture (SCR). Technological developments
  including camera traps and GPS telemetry have opened new opportunities
  for studying animal demography and movement, yet statistical
  models for these two data types have largely developed along
  parallel tracks. We present a hierarchical model in
  which both datasets are conditioned on a movement process for a
  clearly-defined population. We fit the model to data from 60 camera
  traps and 23,572 GPS telemetry locations collected on 17 male
  white-tailed deer in the Big Cypress
  National Preserve, Florida, USA during July 2015. Telemetry data were
  collected on a 3-4 hr acquisition schedule, and we
  modeled the movement paths of all individuals in the region with a Ornstein-Uhlenbeck process that
  included individual-specific random effects. Two of the 17 deer with
  GPS collars were detected on cameras. An additional 20 male deer without
  collars were detected on cameras and individually identified based
  on their unique antler characteristics. Abundance was 126 (95\% CI:
  88--177) in the 228 km$^2$ region---only slightly higher than
  ...
  movement rates and space use as revealed by the joint model. 
  Joint modeling allowed the telemetry data to inform the movement
  model and the SCR encounter model, while leveraging information in 
  the camera data to inform abundance, distribution, and movement.
  Unlike most existing methods for population-level inference on
  movement, the joint SCR-movement model can yield unbiased
  inferences even if non-uniform sampling is used to deploy
  transmitters. Potential extensions of the model include the addition
  of resource selection parameters, and relaxation of the closure
  assumption when interest lies in survival and recruitment. These
  developments would contribute to the emerging holistic framework for
  the study of animal ecology---one that uses modern technology and
  spatio-temporal statistics to learn about interactions between behavior and demography.
%\end{onecolabstract}
\end{abstract}
...
that the processes governing abundance and distribution -- %namely:
...
%\citep[but see][]{ergon_gardner:2014,raabe_etal:2014,schaub_royle:2014,dorazio_price:2019}. 
...
% has been criticized \citep{ivan_etal:2013} because it implies
...
% The implications of these simplifying assumptions for inference on
% abundance and distribution have only recently been investigated
% \citep{royle_etal:2016}.  
% Several SCR models have been developed for encounter history data and
% telemetry data, but these models have not been used to draw inferences
% about movement
...
% A natural choice for describing how observation model describing how
...
%the limitations associated with independent analyses of
%capture-recapture and telemetry datasets. % for studying movement and
%distribution provides a motivation for the development of joint models
%for the two types of data \citep[pp. 534--535]{royle_etal:2014}. In
%A joint analysis should not only improve inferences on important
%population-level movement parameters,
...
distribution. 
%and thereby improve understanding of abundance and distribution even
%if space use and movement are not of primary interest.
...
(Table~\ref{tab:defs}). 
%An activity center represents the expected location of an individual
%during the time period of interest.
Depending on the species, activity centers over the course of the
survey might correspond to territory centers or %, or more generally, 
...
centers.
%Modeling spatial variation in the density of
%individuals (instead of activity centers) during a specific time
%interval, moveme
...
data augmentation, % \citep{royle_etal:2007},
...
adopt %a discrete-time formulation of
...
%${\bm B} = \mathrm{diag}(-\beta)$ and
${\bf B} = \rho \mathbf{I}$ and
%${\bm \Sigma} = \mathrm{diag}(\sigma^2)$.
${\bm \Sigma} = \sigma^2\mathbf{I}$. % where $\rho = \exp(-\beta)$.
This formulation of the OU model is similar to a biased random walk, 
% \citep{mcclintock_etal:2021}.
...
be no movement). 
% The variance of the bivariate normal stationary
% distribution (i.e., the utilization distribution) is $\sigma^2$.
...
% Several options exist for estimating home range size from the
% partially observed movement paths. These options can be categorized 
% based on whether the focus is on the area containing the
% \emph{realized} path or the area of the
% % utlization distribution, which describes the
% \emph{expected} locations.

% Quantifying the area of the realized path is impossible because the
% path itself has no width. However, many methods have been developed to
% describe the area containing the path, with the minimum convex polygon
% (MCP) method being among the most popular. In this context, the
% MCP would include both observed and estimated locations along the
% partially-observed movement path. In a Bayesian analysis, this would
% involve computing the MCP for each posterior sample of the path,
% thereby producing posterior distributions for the home range area of
% each individual in the population. These posterior distributions of
% home range size would naturally account for the area used, but not
% observed to have been  used, between telemetry fixes. %No arbitrary
%assumptions about bandwidth parameters would need to be made, as is
%required by kernel density estimation approaches. Rather, the
%``bridge'' between observed locations is imputed based upon the
%parameters of the estimated movement model.
...
the UD, for example to assess the size of 50\% core areas. %Unlike
%Brownian bridge models, the path between two locations depends on both
%the telemetry data and the camera data (see below), and bridge
%distribution is therefore not multivariate normal in general.
...
\begin{equation}
  % p_{ijt} = 1-\exp(-g_0\exp(-\|\bx_{j}-\buit\|^2)/(2\sigma_{\rm det}^2))
...
\subsubsection{Transmitter deployment process}% and non-uniform sampling}
...
transmitters. Although there may be no recapture data, %and this
% information would not be sufficient for estimating abundance,
...
the camera-based encounter histories. Specifically, %we let $n$ denote
%the number of individuals that were either captured during transmitter 
%deployment or detected for the first time during the camera
%survey. Then,
...
% Moreover, modeling
% the transmitter deployment process makes it possible to  alleviate
% bias arising from non-uniform inclusion probabilities, for example
% when animals are easier to capture in more open habitats than in areas
% with denser vegetation. All that is needed is to model $p^{\rm cap}$
...
to use a multinomial likelihood in which %the number of individual not detected
...
\begin{multline}
%  p(y^{\rm cap},y,n|\{\bu_{i1}, \dots, \bu_{iT}\}, \bsi, N) = \\
%  p({\bm y}^{\rm cap},{\bm y},n|\{\bu_{it}\}, \{\bs_i\}, N) = \\
  p(\{ y_i^{\rm cap}\},\{ y_{ijt}\},n|\{\bu_{it}\}, \{\bs_i\}, N) = \\
  % \Bigg(\frac{N!}{n_0!}\Bigg)\left(\prod_{i=1}^n
  %   \mathrm{Bern}(y_i^{\rm cap}|p^{\rm cap}_i) \left\{\prod_{j=1}^J\prod_{k=1}^K
  % \mathrm{Bern}(y_{ijk}|p_{ijk}) \right\}\right)
  % \Bigg(1-p^*\Bigg)^{n_0}
  \left\{
  \prod_{i=1}^n
...
%Code is presented in Appendix XXX.
...
%which in this case Eq.~\ref{}, directly
...
surrounding $\bsi$. %A comparison of the marginalization approaches is
...
capture process. 
% as described in Sec.~\ref{ssec:tr-deploy}.
%We assumed $p^{\rm cap}$ was constant
%because the open habitat conditions that permitted
%aerial capture.
...
% The online repository also includes a simple simulation study
% assessing bias, variance, and coverage of the posterior mode as a
% point estimator of abundance (Appendix S2).
...
%, which likely resulted from variation in bedding and foraging behavior.
...
% Telemetry data provided direct information about encounter rate. The
% individuals who spent most of their time far from the cameras were
% never detected. The two individuals with collars that were close to
% the camera array evaded detection at some locations, even when they
% were known to be within 100 m of a camera (Fig~\ref{fig:path-cam}).
...
time-invariant encounter rates. % as a simple function of the distance
% between traps and activity centers. % Accounting for this source of
% variation increased the estimate of abundance as expected, but the
% reason for the slight changes was...

The baseline encounter rate of $\lambda_0=0.952$ (Table~\ref{tab:post}) %(0.579--1.45)
...
precision between the models with and without telemetry data. %Another
%benefit of modeling detection conditional on location is that 
...
\begin{table}[h!]
  ...
  \caption{Posterior summary statistics for the spatial
    ...
    % : expected value of density, deer/km$^2$
    % (E($D$)), realized value of abundance ($N$), autocorrelation parameter of
    % movement model ($\rho$), mean and standard
    % deviation of the log-normal distribution for ${\sigma_i}$,
    % baseline encounter rate ($\lambda_0$), scale parameter of local
    % detection function ($\sigma_{\rm det}$), and capture probability
    % for telemetry deployment ($p_{\rm cap}$).

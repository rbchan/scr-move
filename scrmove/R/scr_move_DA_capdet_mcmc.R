...
scrMoveDA <- function(ycap,  ## nx1 initial (helicopter-based) capture histories
...
##        else
    ...
    ld.u[,1] <- rowSums(dnorm(u[,1,], s[1:n,], sigma, log=TRUE))
    for(k in 2:K) {
        ld.u[,k] <-
    ...
    colnames(out) <- c("ED", "rho", "sigma", "lam0", "kappa", "psi",
                       "N", "p", "deviance.ycap", "deviance.ydet", "deviance.u")

    if(report>0)
        cat("\nstarting values =",
            round(c(ED, rho, sigma, lam0, kappa, psi, N, p,
        ...
        if(block.rhosigma) {
            ...
            if(sigma.cand>0) {
                ...
                for(i in (n+1):M) {
                    if(z[i]==1) {
                        ld.ydet.aug.cand[i] <- get_ld_ydet_aug(n.mci, s[i,1], s[i,2],
                                                               x, K, sigma.cand, rho, lam0, kappa,
                                                               trim^2, notOper, nthreads)
                    }
                }
                ld.sigma <- ld.sigma.cand <- 0
                mhr.sigma <-  exp((ld.u.cand.sum + sum(ld.ydet.aug.cand) + ld.sigma.cand) -
                                  (ld.u.sum + sum(ld.ydet.aug) + ld.sigma))
...
##                ld.ydet.cand <- get_ld_y_nK_stats(ydet, lambda.cand, nthreads) ## avoid R::dpois
        ...
        sM.ups <- 0
        for(i in 1:n) {
            s1.cand <- rnorm(1, s[i,1], tune[7])
            s2.cand <- rnorm(1, s[i,2], tune[7])
            if(s1.cand < xlim[1] | s1.cand > xlim[2] | s2.cand < ylim[1] | s2.cand > ylim[2])
                next
            s.cand <- c(s1.cand, s2.cand)
            ld.u.cand[i,] <- ld.u[i,]
            ld.u.cand[i,1] <- sum(dnorm(u[i,1,], s.cand, sigma, log=TRUE))
            ld.u.cand[i,-1] <-
        ...
        ld.ydet.aug.cand[(n+1):M] <- 0
        for(i in (n+1):M) {
            s1.cand <- rnorm(1, s[i,1], tune[7]*10)
            s2.cand <- rnorm(1, s[i,2], tune[7]*10)
            if(s1.cand < xlim[1] | s1.cand > xlim[2] | s2.cand < ylim[1] | s2.cand > ylim[2])
                next
            s.cand <- c(s1.cand, s2.cand)
            if(z[i]==1) {
        ...
        for(k in 1:K) {
            if(k==1) {
                ...
                ld.u.cand[,k] <- rowSums(dnorm(u.cand.k, s[1:n,], sigma, log=TRUE))
            } else {
                if(propose.u.from.prior[k]) {
                    u.cand.k <- cbind(
                        rnorm(n, s[1:n,1]+(u[,k-1,1]-s[1:n,1])*rho, sqrt(sigma^2-sigma^2*rho^2)),
                        rnorm(n, s[1:n,2]+(u[,k-1,2]-s[1:n,2])*rho, sqrt(sigma^2-sigma^2*rho^2)))
                } else {
                    u.cand.k <- u.cand[,k,]
                }
        ...
        out[iter,] <- c(ED, rho, sigma, lam0, kappa, psi, N,
